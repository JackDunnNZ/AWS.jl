...
Begins setup of time-based one-time password multi-factor authentication (TOTP MFA) for a
...
- `"UserAttributeUpdateSettings"`: The settings for updates to user attributes. These
  ...
  address and phone number attributes. For more information, see  Verifying updates to to
...
- `"ExplicitAuthFlows"`: The authentication flows that are supported by the user pool
  ...
  passwords.    ALLOW_USER_SRP_AUTH: Enable SRP-based authentication.
  ALLOW_REFRESH_TOKEN_AUTH: Enable authflow to refresh tokens.   If you don't specify a value
  for ExplicitAuthFlows, your app client activates the ALLOW_USER_SRP_AUTH and
  ALLOW_CUSTOM_AUTH authentication flows.
...
- `"SupportedIdentityProviders"`: A list of provider names for the IdPs that this client
  supports. The following are supported: COGNITO, Facebook, Google LoginWithAmazon, and the
  names of your own SAML and OIDC providers.
